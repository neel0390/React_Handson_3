...
function App() {
  ...
  const changeHandler = (e) => {
    //parameter 'e' is an object that represents the event that has occured
          ...
          <div className="container">
            {employees.map((value, index) => {
              return (
                ...
                </div> //giving an unique key and iyerating through array
              );
            })}
          </div>
          <br />
          <button type="button" onClick={() => setUserDetails(true)} className="btn1">
            Go Back
          </button>
        </div>
      )}
